...
#include "headers/glcd_include/glcd_Device.h"
#include "headers/glcd_include/glcd_io.h"
#include "headers/glcd_include/glcd_errno.h"
...
{  
	 ...
	 *
	 * Note: the reason that SetPixels() below always uses GLCD_WHITE, is that once the
	 * the invert flag is in place, the lower level read/write code will invert data
	 * as needed.
	 * So clearing an areas to GLCD_WHITE when the mode is INVERTED will set the area to GLCD_BLACK
	 * as is required.
	 */

	this->SetPixels(0,0, DISPLAY_WIDTH-1,DISPLAY_HEIGHT-1, GLCD_WHITE);
	this->GotoXY(0,0);
