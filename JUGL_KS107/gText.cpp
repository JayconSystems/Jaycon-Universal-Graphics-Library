...
#include "headers/glcd_include/gText.h"
#include "headers/glcd_Config.h" 
...
void gText::ClearArea(void)
{
	...
	glcd_Device::SetPixels(this->tarea.x1, this->tarea.y1, 
		this->tarea.x2, this->tarea.y2, 
			this->FontColor == GLCD_BLACK ? GLCD_WHITE : GLCD_BLACK);
	/*
...
void gText::SpecialChar(uint8_t c)
{
	...
	if(c == '\n')
	{
		 ...
		 * It looks better when using inverted (GLCD_WHITE) text, on proportional fonts, and
		 * doing GLCD_WHITE scroll fills.
		 *
		 */


		if(this->x < this->tarea.x2)
			glcd_Device::SetPixels(this->x, this->y, this->tarea.x2, this->y+height, this->FontColor == GLCD_BLACK ? GLCD_WHITE : GLCD_BLACK);
...
#endif
		{
			...
			if(this->y + 2*height >= this->tarea.y2)
			{
				 ...
				 * operation will not be colored GLCD_BLACK for the full height of the character.
				...
				this->ScrollUp(this->tarea.x1, this->tarea.y1, 
					this->tarea.x2, this->tarea.y2, pixels, this->FontColor == GLCD_BLACK ? GLCD_WHITE : GLCD_BLACK);
...
/*================== NEW FONT DRAWING ===================================*/
	...
	for(p = 0; p < pixels;)
	{
		 ...
		 *
		 * Then it is easy to paint the desired bits since if the color is GLCD_WHITE,
		 * the paint bits are set, and if the coloer is not GLCD_WHITE the paint bits are stripped.
...
void gText::EraseTextLine( eraseLine_t type) 
{
	...
	uint8_t y = this->y;
	uint8_t height = FontRead(this->Font+FONT_HEIGHT);
	uint8_t color = (this->FontColor == GLCD_BLACK) ? GLCD_WHITE : GLCD_BLACK;
 ...
 *
 * @param font a font definition
 * @param color  can be GLCD_WHITE or GLCD_BLACK and defaults to black
 ...
 *
 * @note
 * When the display is initilized in normal mode, GLCD_BLACK renders dark 
 * pixels on a white background and GLCD_WHITE renders white pixels on 
 ...
 * Select a font color
 *
 * @param color  can be GLCD_WHITE or GLCD_BLACK
 *
 *
 * @see SelectFont()
 * @see SetTextMode()
